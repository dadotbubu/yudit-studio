...
규칙: 인사·칭찬·격려 빼고 바로 본론. 완성 문장을 대신 써주지 마라 — 어디를 어떻게 바꿀지 방향만 짚고, 최종 문장은 작성자가 직접 쓰게 둔다. 좋은 문장은 "그대로 OK"로 두고 건드리지 마라.
...
[출력 — 인사치레 없이, 위에서 아래로 구간별]
① 구간별 피드백:
   원본: (대사 한 구간 인용)
   → 이렇게: (구체 방향·제안)        또는  → 그대로 OK        또는  → 삭제
   (왜: 어떤 잣대인지 한 줄. 필요하면 [장면] 연출 메모)
   ※ 완성 문장 대필 금지 — 방향만 짚어라. 좋은 건 "그대로 OK".
② ⓢ 훅 서브자막 제안 (작성자가 자주 빠뜨림 — 꼭): 첫 훅을 보강하는 강력한 한 문장 1~2개 (핵심·숫자·결과·권위·반전을 화면에 박기). 영상 내내 갈지 훅에서만 쓸지는 작성자가 판단.
③ 마지막 공유 자료 제안 (한 개 말고 여러 개·다양한 뷰로 깔아준다):
   이 주제면 댓글·DM로 주면 저장·전환이 터질 자료를 3개 이상 제안.
   ★ 같은 주제도 여러 관점(뷰)으로 쪼개 제안 — 초보용/고수용, 입문 체크리스트/디테일 비교표, 단계별 가이드 등.
     한 주제로 공유 자료를 여러 개 만들 수 있게 옵션을 펼쳐준다.
   유형도 섞어서 — 체크리스트 / 앱·도구 정보 / 템플릿·양식 / 정리 리스트 / 신청링크 모음 등.
   각각 한 줄로 (무슨 자료 + 어떤 뷰·대상 + 왜 먹히는지).
   (실제 자료가 뭐가 있는지 모르면, 그 주제에서 흔히 쓰이는 도구·양식을 구체 이름까지 조사해서 제안)`;
}
