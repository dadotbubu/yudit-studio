...
[★★ 먼저 — 재료부터 본다. 앵글부터 열지 마라 (정본: 릴스/가이드/후킹_마스터_가이드.md §0-4)]
 ...
 ★★★ 산출 형식 — 훅마다 «근거» 칸을 붙여라. 비면 그 안은 내지 마라.
   N. [각도 이름] / 훅: ___ / 결론: ___ (이 릴스가 결국 하는 말 한 줄) / 근거: (그 말이 어디 있나 — 원본 ○번째 줄·본론 ① 안·에피○○) / 그릇: [20] + 그 레퍼 훅 «원문 문장»까지 같이 적어라(번호만 ✕ — 확정 뒤 훅이 표류하는 걸 막는다). 승격뿐이면 비운다 / 왜: (한 줄)
...
[출력]
① 게이트 점검: ${gateBody}
② 재료판 → 「🎯 주제: ___ + 나의 경험담」 한 줄 → ✕ 줄에서 훅 3~5개 (재료가 하나여도 그릇을 바꿔 3개는 낸다 — 앵글 목록을 순서대로 채우지 마라. 앵글은 이름표다). 세트는 깔끔하게 (점검은 적지 말 것):
   N. [앵글명] 훅: ___  /  결론: ___
      근거: ___  /  왜: ___
   ※ 각 세트는 속으로 5항목(주제·권위·타겟·이득·창작/유디트화)을 점검해, 통과한 것만 낸다. ✕면 다시 창작.
③ 맨 끝에 점검 요약 한 줄만: "낸 세트 전부 5항목(주제·권위·타겟·이득·창작/유디트화) 통과" — 재료가 모자라 못 낸 각도가 있으면 여기 짧게 덧붙인다.
