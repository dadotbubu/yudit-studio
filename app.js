...
규칙: 인사·칭찬·격려 빼고 바로 본론. 각 구간을 어떻게 고칠지 ★고친 문장 예시까지 같이 보여줘라★ (방향만 말하면 무슨 뜻인지 안 와닿는다). 단 전체 대본을 통으로 새로 쓰진 말고 구간별로만 짚어라. 좋은 문장은 "그대로 OK"로 두고 건드리지 마라.
...
① 구간별 피드백 — 아래 표로 (대사를 위에서 아래 순서대로 한 행씩):
| 원본 대사 | 이렇게 고쳐요 (고친 문장) | 왜 / ⓢ서브자막 |
|---|---|---|
- '이렇게 고쳐요' 칸엔 방향만 말고 ★실제로 고친 문장★을 써라. 그대로 좋으면 "그대로 OK", 뺄 거면 "삭제".
- '왜' 칸엔 짧은 이유 한 줄. 그 구간에 어울리는 서브자막 후보 있으면 'ⓢ ___'도 같이.

② ⓢ 훅 서브자막 — 표로:
| 서브자막 후보 | 노린 것 |
|---|---|
→ 첫 훅을 보강하는 강력한 한 문장 1~2개 (숫자·결과·권위·반전을 화면에 박기). 영상 내내 갈지 훅에서만 쓸지는 작성자 판단.

③ 마지막 공유 자료 제안 — 표로 (여러 개·다양한 뷰):
| 자료 (유형) | 뷰·대상 | 저장/공유 | 왜 먹히나 |
|---|---|---|---|
→ 같은 주제를 여러 관점으로 쪼개 3개 이상 (초보용/고수용 등). 유형 섞어서 — 체크리스트/비교표/템플릿/앱정보/링크모음.
→ '저장/공유' 칸: 저장↑(내가 두고 쓰는 실용 자료 — 체크리스트·템플릿·정리표) / 공유↑(남한테 보내는·대상 있는·공감 자료) / 둘 다. (저장과 공유는 결이 다르니 구분해서 표시)
→ 표 맨 위에 ★ 제일 저장·공유 터질 자료 1개를 콕 추천.
→ 실제 도구·양식이 뭐가 있는지 모르면, 그 주제에서 흔히 쓰는 것 구체 이름까지 조사해서 제안.`;
}
