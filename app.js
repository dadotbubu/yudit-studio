...
// ===== 1단계: 훅·표지 프롬프트 =====
// ⚠️ 임시: 스킬(`릴스-대본`)·`scripts/기획규칙.py`에 옛 훅 규칙이 남아 있어,
//    아래 code 프롬프트에 "스킬의 훅 규칙은 따르지 마라"를 박아 막고 있다.
//    스킬을 정리하면 그 문단(★★로 시작)을 삭제할 것. 상세는 plan.md 「지금 임시 상태」.
...
function plBuildHookPrompt(mode = 'chat') {
  ...
  const HM = D.hookMethod;
  const HSR = D.hookSentenceRules;
  // ★규칙을 늘리지 마라. 늘릴수록 앤이 생각을 안 하고 칸만 채운다.
  //   조합·재료·역할분담·점검은 planning_data.js에 남겨두고 프롬프트에는 싣지 않는다.
  const coreBlock = `[말투] 유디트 말투 — ~예요 / ~거든요 / ~더라고요, 1인칭 경험. 훅(첫 3초)은 반말도 OK.
✕ '~습니다' 문어체로 8개를 도배하는 것 — 그건 유디트 문장이 아니다.

[순서 — 앵글에서 출발하지 말고 사실에서 출발한다]
1. 사실 5줄을 적는다 — 훅처럼 쓰지 말고 사실로. 유디트가 준 것만 쓰고 나머지는 비운다
${HM.facts5.map((f, i) => `   ${i + 1}) ${f}`).join('\n')}
   → 3·4·5번이 훅 재료다. 1·2번은 본문 재료다. 모르는 숫자는 [ ]로 비운다
2. 어긋남 2~3개를 고른다 — 정보 많은 줄이 아니라 '어?' 싶은 이상한 줄. 보통 3·4·5번에서 나온다
3. 그 어긋남으로 훅 8개 — 1개를 잘 쓰려 하면 막힌다. 8개 쓰고 고른다
4. 추천 1~2개 + 왜 그걸 골랐는지
5. 표지는 추천한 훅에만 — 그 훅이 말하지 않은 알맹이를 준다

[금지 — 이 셋만]
✕ 소재·방법·개수를 훅에 넣지 마라 — '마일리지 적립 7가지 방법'이 소재면 훅에 '7가지 방법'이 없다. 방법은 본문 몫, 훅은 어긋남 몫이다
✕ 유디트 실제 수치가 아닌 숫자를 지어내지 마라 — [ ]로 비워 유디트가 채우게
✕ 사실 5줄을 지어내지 마라 — 유디트가 준 것(주제·경험 칸)에서 안 나오는 줄은 [ ]로 비우고 질문으로 만들어라. 그럴듯하게 채우면 가짜 사실 위에 훅을 짓게 된다
   → 각 줄 끝에 출처를 [유디트] 또는 [질문]으로 표시하라. [질문]인 줄은 훅 재료로 쓰지 마라
   → 훅 재료(3·4·5)에 [유디트]가 하나도 없으면 훅을 내지 말고 질문 3개만 던져라`;
  const sentBlock = `[문장 전환 — 훅 문장을 마감할 때]
${HSR.switches.map(s => `- ${s.name}`).join('\n')}
...
★ 먼저 \`릴스-대본\` 스킬을 열고 「시작 전에 찍는 것」을 그대로 찍어라 — 세계관·서사라이브러리·말투. 기억으로 쓰지 마라.
   \`scripts/훅모음.py --카테고리 ${cat}\` 도 훅을 짜기 전에 돌린다 (레퍼 훅 실물을 보려고).
   스킬을 못 열면 그 사실을 맨 앞에 적어라 — 세계관 없이 쓴 훅인지 유디트가 알아야 한다.
   (planning_data.js 를 통째로 Read 하지 말 것 — 157KB 중 절반이 레퍼라 규칙만 못 본다.)

★★ 훅 순서·규칙은 아래가 정본이다. 스킬이나 \`scripts/기획규칙.py\`에 훅 규칙이 따로 있어도
   그건 옛 버전이니 따르지 마라 ("13앵글을 순서대로 다 채워라", "표지·훅 둘 다 주제·권위·이득"은 폐기).

${coreBlock}

${sentBlock}

[훅 문장] 대사체, 한 호흡에 끝나는 짧은 한 문장(20자 내외). 두 문장으로 늘이지 마라 — 설명·대비·이유는 본문 몫이다.
※ 레퍼처럼 짧게: "이거 사세요" "안 싸우냐고요?" "어떻게 5번이나 합격했을까?"

[앵글 — 메뉴다. 다 채우지 말고, 여기 없는 이름을 지어내지도 마라]
${D.hookAngles.map(a => a.name).join(' · ')}

[레퍼] 우리 레퍼가 이미 쓴 각도면 재탕이니 빼라. 뺐으면 어느 레퍼와 겹쳤는지 한 줄로 적어라.

[출력] ① 사실 5줄 ② 어긋남 2~3개 ③ 훅 8개(각 줄 앞에 앵글명) ④ 추천 1~2개 + 이유 ⑤ 추천 훅의 표지 2개씩
※ 점검 문구를 길게 적지 마라. 규칙을 지켰다고 스스로 선언하는 줄은 필요 없다 — 훅 문장으로 보여라.

[목적] ${purpDef.name}
[주제] ${topic}
[내 경험] ${exp || '(없음 — 훅 재료가 비면 훅 대신 질문 3개를 던져라)'}`;
  // 같은 목적 레퍼 훅 5개 + 터진 이유
  ...
  const angles = D.hookAngles.map(a => a.name).join(' · ');
  return `너는 인스타그램 릴스 훅 기획 에이전트다. 주어진 주제로 스크롤을 멈추게 하는 첫 3초 훅을 만든다.
...
[주제] ${topic}
[목적] ${purpDef.name}
[내 경험·에피소드] ${exp || '(없음 — 훅 재료가 비면 훅 대신 질문 3개를 던져라)'}

${coreBlock}

${sentBlock}

[훅 문장] 대사체(구어체), 한 호흡에 끝나는 짧은 한 문장(20자 내외). 두 문장으로 늘이지 마라 — 설명·대비·이유는 본문 몫이다.
※ 레퍼처럼 짧게: "이거 사세요" "안 싸우냐고요?" "어떻게 5번이나 합격했을까?" "목걸이 샀더니 주식이 들어왔어요"

[표지] 제목체(명사형 OK), 15자 이내. 결과·숫자·리스트·대상 중 하나 이상은 들어간다.

[앵글 — 메뉴다. 다 채우지 말고, 여기 없는 이름을 지어내지도 마라]
${angles}

[검증된 레퍼 — 문장은 베끼지 말고 왜 멈췄는지만 가져와라]
${refEx}
※ 우리 레퍼가 이미 쓴 각도면 재탕이니 빼라. 뺐으면 어느 레퍼와 겹쳤는지 한 줄로 적어라.

[출력]
① 사실 5줄
② 어긋남 2~3개 — 왜 이상한지 반 줄씩
③ 훅 8개 — [앵글명] 훅 문장
④ 추천 1~2개 + 왜 그걸 골랐는지 한 줄씩
⑤ 추천한 훅의 표지 2개씩
※ 점검 문구를 길게 적지 마라. 규칙을 지켰다고 스스로 선언하는 줄은 필요 없다 — 훅 문장으로 보여라.
※ 마음에 안 들면 사용자가 "더" 또는 특정 어긋남을 말한다. 그때마다 앞서 낸 것과 겹치지 않게 새로 창작하라.`;
}
