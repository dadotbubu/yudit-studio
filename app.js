...
※ 레퍼 68편의 「왜 터졌나」를 전수로 훑으니 가운데 줄들이 전부 같은 장치 여덟 개였다.
※ 훅만 좋고 본론이 맨손이면 죽는다. 실제로 죽은 세 편은 채점하면 장치가 0개였다.
 ① 환산 — 숫자를 혼자 두지 않는다 (레퍼 12편, 최다)
...
13. ★★★본론 장치를 세어라 (2026-08-08 신설 · 레퍼 68편 전수 — 지금 제일 잘 걸리는 자리다).
    터지는 릴스의 본론은 전부 아래 여덟 개 위에 있었고, 죽은 대본은 0개였다. 훅이 멀쩡해도 여기가 비면 죽는다.
    ① 환산(숫자를 혼자 두지 않는다 — 7,300원인데 나는 3,600원 / 21만 마일 = 유럽 왕복 3번)
    ② 메커니즘(주장 뒤에 「~때문이에요」 — 이유가 붙어야 저장된다)
    ③ 설계도(행동 경로를 끝까지 — 어디를 눌러서 뭘 하는지)
    ④ 사용 장면(항목마다 「언제 쓰는지」)
    ⑤ 내 선택이 증거(「저희는 ~때문에 이렇게 했어요」 — ★「때문에」가 없으면 안 센다)
    ⑥ 반론 선차단(핑계·단점을 내가 먼저)
    ⑦ 빈칸(★혜택을 끝까지 말한 뒤에 이름만 가린다 — 설명 얇은 채 개수만 던지면 안 센다)
    ⑧ 재정의(「A가 아니라 B다」)
