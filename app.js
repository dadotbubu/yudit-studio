    ...
    <div class="p-2 md:p-6 space-y-3 md:space-y-6">
                ...
                ${scriptRows.map((row, idx) => `
                  <tr class="border-t border-botanical-stone group">
                    <td class="px-4 py-3 font-semibold relative">
                      <button onclick="removeScriptRow(${content.id}, ${idx})" title="행 삭제" class="absolute bottom-1 left-1 w-5 h-5 rounded text-xs text-red-400 opacity-0 group-hover:opacity-100 hover:bg-red-50 transition-opacity">×</button>
      ...
      <div class="md:border md:border-botanical-stone md:rounded-xl p-0 md:p-5">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0 mb-4">
          ...
          <button onclick="copyCaption(${content.id})" class="w-fit px-3 py-1 rounded-full text-xs border border-botanical-stone hover:bg-botanical-cream transition-all">캡션 복사</button>
        </div>
