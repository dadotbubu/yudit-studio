...
function plBuildHookPrompt(mode = 'chat') {
  ...
  // 순서 블록 — 앵글이 아니라 사실에서 출발한다 (끼워맞추기 방지)
...
※ ${HSR.note.split('★')[1] || ''}`;
  ...
  const outBlock = `[출력 — 이 순서 그대로]
① 게이트 점검: ${gateBody}
② 사실 5줄 — 주제·경험에서 뽑아 그대로 적는다. 훅처럼 쓰지 말고 사실로. 모르는 숫자는 [ ]로 비운다
③ 어긋남 2~3개 — ②에서 고른다. 왜 이상한지 반 줄씩
④ 레퍼 긴장 — 참고 레퍼가 만든 찝찝함 한 줄씩. 이 주제에서 안 생기면 "버림", 우리 레퍼가 이미 쓴 각도면 "재탕 — 뺌"이라고 적는다
⑤ 훅 8개 — [앵글명] 훅 문장. 고른 어긋남에 맞는 앵글만 쓴다(앵글 순회 금지). 20자 내외, 한 호흡
⑥ 추천 1~2개 + 왜 그걸 골랐는지 한 줄씩. 나열만 하고 끝내지 마라
⑦ 표지 — ⑥에서 추천한 훅에만, 각 2개씩. 그 훅이 말하지 않은 알맹이를 준다
⑧ 점검 한 줄 — 8개 전부 문장 전환 통과 / 쉽다 통과인지`;
...
★★ 훅 짜는 순서·규칙은 **아래가 정본이다.** 스킬이나 \`scripts/기획규칙.py\`에 훅 규칙이
   따로 있어도 그건 옛 버전이니 따르지 마라. 특히 "13앵글을 순서대로 다 채워라",
   "표지·훅 둘 다 주제·권위·이득이 드러나야 한다"는 **폐기됐다.** 아래 순서대로만 간다.
...
[쉽다 체크 — 순서 7]
훅이 세도 "나는 못 하겠는데"가 들면 스크롤된다. 진입장벽(돈·시간·자격)을 훅이나 표지에서 한 번 낮춰라.

${outBlock}

[목적] ${purpDef.name}
[주제] ${topic}
[내 경험] ${exp || '(없음 — 사실 5줄에서 모르는 칸은 [ ]로 비우고 유디트가 채우게)'}`;
  // 같은 목적 레퍼 훅 5개 + 터진 이유 (긴장 학습용)
...
[주제] ${topic}
[목적] ${purpDef.name}
[내 경험·에피소드] ${exp || '(없음)'}${exp ? ' — 사실 5줄의 재료다. 여기 없는 숫자는 지어내지 말고 [ ]로 비워라' : ''}

${methodBlock}

[검증된 레퍼 — 문장·패턴이 아니라 긴장만 가져와라]
${refEx}
※ 각 레퍼가 만든 찝찝함(그 훅을 보고 시청자 머리에 뜬 질문)이 뭔지 먼저 적어라. 그게 이 주제에서도 생기면 쓰고, 안 생기면 그 레퍼는 버린다.

[훅 앵글 — 메뉴다. 골라 쓰는 것이지 다 채우는 게 아니다]
${angles}
...
[훅 점검 — 4개만 본다]
- 어긋남: "어?"가 오나. 정보만 있으면 훅이 아니다
- 창작: 참고 레퍼와 같은 소재면 치환 금지(카피)·반드시 새로 창작
- 유디트화: 유디트 말투·경험이 묻어나나
- 문장 전환: 주어가 '나'거나 '알려드릴게요·해보세요·저장하세요'로 끝나지 않나
※ 주제·권위·이득은 훅 점검 항목이 아니다 — 그건 표지와 인트로가 나눠 갖는다

${outBlock}
⑨ 마음에 안 들면 사용자가 "더" 또는 특정 어긋남(예: "3번 어긋남으로 더")을 말한다. 그때마다 앞서 낸 것과 겹치지 않게 계속 새로 창작하라.`;
}
