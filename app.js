...
★★ 앵글을 만들기 전에 §0 화자 게이트부터 통과해라 (\`가이드/후킹_마스터_가이드.md\` §0·§0-2·§0-3).
...
그 감각으로 이 주제 훅을 뽑아줘. 재료판을 먼저 내고 → ✕ 줄에서 훅 3~7개, 훅마다 «결론 한 줄»(이 릴스가 결국 하는 말)을 붙여서. 표지 «문장»은 지금 쓰지 마 — 대본이 확정된 뒤 §표지 완성에서 쓴다. 차별점은 첫 문장에 하나만, 5요소(주제·권위/타겟·이득·창작·유디트화)는 속으로 점검해 통과한 것만(점검은 끝에 한 줄 요약).
...
[★★ 먼저 — 재료부터 본다. 앵글부터 열지 마라 (정본: 릴스/가이드/후킹_마스터_가이드.md §0-4)]
 ...
 ★★★ 산출 형식 — 훅마다 «근거» 칸을 붙여라. 비면 그 안은 내지 마라.
   N. [각도 이름] / 훅: ___ / 결론: ___ (이 릴스가 결국 하는 말 한 줄) / 근거: (그 말이 어디 있나 — 원본 ○번째 줄·본론 ① 안·에피○○) / 왜: (한 줄)
...
- 주제: 무엇에 대한 얘긴지 한눈에 잡히나
- 권위 or 타겟: 누가(경험·자격) 또는 누구한테(대상) 하는 말인지 드러나나
...
[출력]
① 게이트 점검: ${gateBody}
② 재료판 → ✕ 줄에서 훅 3~7개 (개수는 재료가 정한다 — 앵글 목록을 순서대로 채우지 마라. 앵글은 이름표다). 세트는 깔끔하게 (점검은 적지 말 것):
   N. [앵글명] 훅: ___  /  결론: ___
      근거: ___  /  왜: ___
