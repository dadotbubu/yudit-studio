  ...
  return `
    <div class="p-2 md:p-6 space-y-3 md:space-y-6">
      ...
      <div class="md:border md:border-botanical-stone md:rounded-xl p-0 md:p-5">
        ...
        <textarea id="caption-${content.id}" rows="3" oninput="autoResize(this);updateCaptionText(${content.id}, this.value)" placeholder="못다한 얘기 / 당장 쓸 팁 / 솔직한 생각 등" class="auto-grow unified-text w-full px-3 py-2 rounded-lg border border-botanical-stone focus:outline-none focus:border-botanical-sage resize-none overflow-hidden">${currentCaptionText}</textarea>
      </div>`;
      })()}
      ...
      ${(() => {
        ...
        const boxes = [
          { field: 'pinnedComment', el: 'pinned', s: slot('pinned', 2, '고정 댓글'), ph: "시청자가 할 법한 한 문장 — '나도요'가 붙을" },
          { field: 'regramNote', el: 'regram', s: slot('regram', 3, '리그램 한마디'), ph: '반응 좋은 편만 — TMI 한 줄' },
          { field: 'storyNote', el: 'storynote', s: slot('story', 4, '스토리 한마디'), ph: '만든 이유 + 누가 봐줬으면' },
          { field: 'storyComment', el: 'storycmt', s: slot('storyReply', 5, '스토리 댓글'), ph: 'ㅇ/ㄴ로 답할 수 있는 한 줄' },
        ];
        return `
      <div class="md:border md:border-botanical-stone md:rounded-xl p-0 md:p-5">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0 mb-4">
          ...
          <div class="space-y-2">
            ${[0, 1, 2, 3, 4].map(i => {
              ...
              <div class="flex items-start gap-2">
                ...
                <textarea id="reply${i}-${content.id}" rows="1" oninput="autoResize(this);updateCommentReply(${content.id}, ${i}, this.value)" placeholder="${isFixed ? '' : '영상 내용 한 조각 얹어서'}" class="auto-grow unified-text flex-1 min-w-0 px-3 py-2 rounded-lg border border-botanical-stone focus:outline-none focus:border-botanical-sage resize-none overflow-hidden">${val}</textarea>
...
// ===== 소통 (고정댓글·리그램 한마디·스토리 한마디·스토리 댓글) =====
...
// 링크(바이오·자동 DM) 기간은 일 단위. 업로드일로부터 N일.
function adTermEndDays(refDate, days) {
  if (!refDate || !days || !/^\d{4}-\d{2}-\d{2}/.test(refDate)) return '';
  const d = new Date(refDate.slice(0, 10) + 'T00:00:00');
  if (isNaN(d.getTime())) return '';
...
function netFee(gross) {
  const g = gross || 0;
  if (!g) return 0;
  const income = Math.floor(g * 0.03 / 10) * 10;
...
★ 먼저 \`릴스-훅\` 스킬을 열고 「시작 전에 찍는 것」을 그대로 찍어라. 기억으로 쓰지 마라.
   특히 \`scripts/훅모음.py --원문\` (레퍼 훅 원문 70개 전수 = 그릇 창고) 과 \`scripts/기획규칙.py\` 는
   **훅을 짜기 전에** 돌린다. 카테고리로 좁히지 마라 — 문형은 소재를 안 탄다.
   (planning_data.js 를 통째로 Read 하지 말 것 — 157KB 중 절반이 레퍼라 규칙만 못 본다.)
   그 출력은 «네가 읽는 것»이다 — 채팅에 붙여넣지 마라.
   출력 맨 앞에 「📖 찍은 것」 + 「🎯 주제」 두 줄을 박아라.

★★ 훅 «문장»은 발명하지 마라 — 읽은 레퍼 훅 원문을 «그릇째» 빌려서 만든다 (후킹 가이드 §이식).
   원문 통째로 빌리고(패턴·틀 금지), 명사·수치·상황만 갈아끼우고, 힘 내던 자리(~다고? · 물어보길래
   ~했더니 깜짝 놀라더라고요 · ~인데 아직도)는 그대로 둔다. 그릇이 길면 «답 부분»만 줄인다.
   빌린 그릇은 산출 형식의 「그릇:」 칸에 번호만 적어라 (승격뿐이면 비운다 — 강제 아님).

★★ 앵글을 만들기 전에 §0 화자 게이트부터 통과해라 (\`가이드/후킹_마스터_가이드.md\` §0·§0-2·§0-3·§0-4).
   주어는 ㉮내가 한 것 / ㉯너한테 시키는 것 / ㉰너한테 생길 손익 셋 중 하나 — 제도·뉴스가 주어면 반려.
   ㉮ 라도 성과가 숫자로 없으면 안 선다. 표지·훅은 같은 성과 숫자를 반복하고, 방법은 표지에서 가린다.
   「A면 이렇고 B면 저래요」로 대상을 쪼개지 마라.
   ★ 보는 사람은 팔로워가 아니라 «추천 피드에 뜬 낯선 사람»이다 — 2탄이 아니면 권위(5관왕·부부7관왕·14년차·면접관)가
     훅 첫 마디·서브자막·표지 중 한 번은 들려야 한다. 그 편에서 실제로 한 일이 있으면 그게 상수보다 먼저다.

그 감각으로 이 주제 훅을 뽑아줘. 재료판 → «주제 한 줄» → 훅 두 단 묶음(이식 3개 + 순간포착 3개를 «각각» 만든다 · §순간포착), 훅마다 «결론 한 줄»(이 릴스가 결국 하는 말)을 붙여서. 표지 «문장»은 지금 쓰지 마 — 대본이 확정된 뒤 §표지 완성에서 쓴다. 차별점은 첫 문장에 하나만, 5요소(주제·권위·타겟·이득·창작/유디트화)는 속으로 점검해 통과한 것만(점검은 끝에 한 줄 요약).
...
[내 경험] ${exp || '(없음)'}`;
  ...
  return `너는 인스타그램 릴스 훅 기획 에이전트다. 주어진 주제로 스크롤을 멈추게 하는 첫 3초 훅을 창작하고, 훅마다 «결론 한 줄»(이 릴스가 결국 하는 말)을 붙여라. 표지 «문장»은 이 단계에서 쓰지 않는다 — 대본이 확정된 뒤에 완성한다(2단계 끝).
...
[★★★ 그릇 창고 — 레퍼 훅 «원문» 전수. 훅 문장은 여기서 나온다]
훅 문장을 맨손으로 짓지 마라. 문법만 맞고 리듬이 없는 말이 나온다.
아래를 «다 읽고» 이 주제에 말이 되는 것을 골라, 그릇째 빌려서 뉘앙스만 갈아끼워라.
 ★ 카테고리를 가리지 마라 — 문형은 소재를 안 탄다.
   (실제로 커리어 주제 편이 빌린 그릇 셋은 전부 라이프·기타에서 나왔다)
 ★ 원문 «통째로» 빌려라. 「A하는 B는 따로 있다」 같은 추상형으로 뽑지 마라 —
   힘은 뼈대가 아니라 어미·조사·군더더기(~다고? · ~라고요 · ㅎㅎ)에 붙어 있다.
 ★ 갈아끼우는 건 명사·수치·상황뿐. «힘 내던 자리»는 그대로 둔다.
   인생을 바꾸는 독서 방법은 따로 있다고?  →  연봉을 올리는 이직 방법은 따로 있다고?
   친구가 신혼집 채우는데 가전 총 얼마 들었어? 물어보길래 가전 11종에 1596만원이라고 하니 깜짝 놀라더라고요
     →  친구가 이직 준비하는데 얼마나 걸렸어? 물어보길래 3분이라 했더니 깜짝 놀라더라고요
   저희는 올해 5월에 결혼했는데 아직도 8평 월세집에서 살고 있습니다
     →  저는 대기업만 5번 다녔는데 아직도 이직 제안 받고 있어요
 ★★★ 그릇에 «숫자»가 있으면 내 숫자로 채우는 것까지가 그릇 쓰기다. 숫자를 빼면 뼈대만 남고 힘이 통째로 빠진다.
   100대1 경쟁 뚫고 들어간 은행 5개월 만에 퇴사했습니다
     ⛔ 제 경력보다 좋은 자리, 인터뷰까지 보고 거절했습니다   (숫자 0 — 그릇을 안 쓴 것)
     ○  연봉 2억 자리 제안 왔는데 하루 만에 거절했습니다      (두 자리를 다 채웠다)
   · 숫자 자리 «개수만큼» 채운다. 하나 채우고 하나 비우면 반쪽이다.
   · 숫자의 출처는 서사라이브러리 «에피소드»다 — 에피에 있으면 쓰고, 없으면 «묻거나 뺀다».
     지어 넣지 마라. 비운 채로 내지도 마라. 숫자만 알면 살아날 안이면 「이 숫자를 알면 이 안이 산다」로 물어라.
   · 채웠는데 안 어울리면 그것도 내지 마라 — 억지로 끼운 숫자는 안 끼운 것보다 나쁘다.
   · 그릇에 숫자가 없으면 이식본에도 없어도 된다.
 ★ 그릇이 길어도 «끝까지» 빌려라 — 힘은 리듬에서 나오지 길이에서 나오지 않는다.
   짧게 만들 수 있는 자리는 «답 한 곳»뿐이다.
 ★★★ 답 자리 «말고» 뒤가 사라지면 폐기다. 마무리 어미 · 반응 인용 · 「~인데 아직도 ~고 있어요」 같은
   뒷절은 자르지 마라. 「길어서」·「매끄럽게」·「훅은 짧아야 하니까」는 자르는 이유가 안 된다.
 ★★★ 단, 그릇의 «답 자리»가 반전 «해설»이면 줄이는 게 아니라 «잘라서 본문으로» 내려라.
   답이 «놀란 수치»면 남긴다 — 얼마 들었냐고요? 100만원도 안 들었어요 → 얼마나 했냐고요? 3분도 안 걸렸어요
   답이 «반전 해설»이면 자른다 — 어떻게 매번 비즈니스 타냐고요? 사실 저 부자예요! 돈은 없고 마일리지 부자예요
     ⛔ 어떻게 매번 제안이 오냐고요? 사실 저 준비 안 해요, 프로필만 준비돼 있어요
     ○  어떻게 대기업 5번이나 이직했냐고요?        ← 되묻는 데까지가 훅이다
   되묻기형(~냐고요?)은 물음표에서 끊는 게 기본이다. 뒤를 붙이면 답을 미리 준 셈이라 볼 이유가 준다.
 ★ 감정 방향은 뒤집어도 된다 (아직도 8평=자조 → 아직도 제안=자랑). 빌리는 건 문형이지 감정이 아니다.
 ★★ 판정 — 그릇은 «대본 흐름»을 데리고 온다.
   「~는 따로 있다고?」는 감췄다 푸는 흐름, 「물어보길래 ~했더니」는 격차를 푸는 흐름,
   「~인데 아직도」는 그 상태가 유지되는 비결을 푸는 흐름을 요구한다.
   ⇒ 그 흐름을 채울 재료가 없으면 그 그릇은 쓰지 마라.
 ★★★ 그릇을 «무슨 기준으로» 고르나 — 소재가 비슷한 그릇 ⛔ / «말버릇이 살아남는» 그릇 ○.
   확정된 이식 셋은 전부 의미가 안 맞는다(독서→이직 · 가전→이직 · 8평월세→이직 제안). 이은 건 말버릇뿐이다.
   ★★★ 채팅에 내기 직전, 원문과 훅을 나란히 놓고 «겹치는 말»을 먼저 세라.
   훅은 그릇 원문을 거의 그대로 써야 한다. 명사·수치·상황만 바꾸고, 힘 내던 자리(어미·조사·숫자 자리·질문 구조·반응 인용)는 남겨라.
   겹치는 말이 적으면 그릇을 빌린 게 아니라 새로 쓴 것이다. 겹치는 게 어미 하나뿐이면 폐기하라.
 ★★★ 남은 글자는 «원문 형태 그대로»여야 한다. 매끄럽게 다듬었으면 안 빌린 것이다 —
   ~했더니→~하니까 · ~고 있어요→~고 있습니다 · ~다고?→~일까요? · ~더라고요→~더군요 전부 폐기.
   어색해 보여도 원문 어미를 그대로 둬라.
   숫자 자리·질문 구조·반응 인용 중 하나라도 사라지면 폐기하라. 이 검사를 통과한 것만 채팅에 내라.
 ★ 재료에서 끌어올린 줄(승격)과 그릇에서 빌린 문장(이식)을 굳이 나누지 마라 — 한 문장 안에 섞인다.

[★★★ 순간 포착 훅 — 이식과 «같은 개수»로 창작한다]
이식만 내면 그릇에 매몰된다. 이식 3개 + 순간포착 3개를 «한 묶음»으로 내라.
 ★★★ 둘은 «순서»가 아니라 «각각»이다. 1단을 만들고 그걸 바탕으로 2단을 짜는 게 아니다 —
   같은 주제에서 «따로» 만들어 나란히 내라. (「Phase 1/2」는 출력에서 두 단을 가르는 이름표일 뿐이다)
 ★ 이식 후보를 고쳐 쓰지 마라. 같은 주제에서 «새로» 만든다.
 ★ 막연한 상태를 설명하지 말고, 그 상태가 드러난 «한순간»을 써라.
   언제·몇 번·몇 분째·며칠 만에 — 시간이나 숫자를 박고, 그때 사람이 «실제로 한 행동»을 보여준다.
 ★ 숫자만 꾸며 붙이지 마라. 장면이 눈앞에 보여야 한다.
   막연) 여행 갔다 오면 다 까먹는 분      순간) 여행 끝난 지 3일인데, 식당 이름 다시 물어본 적?
   막연) 사진첩에 사진 너무 많죠          순간) 여행 사진 한 장 찾다가 5분째 사진첩 올린 적 없어요?
 ★ 말끝을 「~하신 분」으로 통일하지 마라. ~한 적? · ~하고 계세요? · 질문 · 혼잣말 — 장면에 맞춰 바꾼다.
 ★ 숫자·시간·상황은 확인된 사실이나 «일반 시청자» 장면에만 써라. 사용자의 경험을 지어내지 마라.
 ★ 레퍼 원문 유지·겹치는 말 검사는 이식 단에만 걸린다. 순간포착 단엔 「그릇」·「남은자리」 칸이 없다.
 ★ 그 밖의 게이트는 그대로다 — 0단계 화자 · 권위 · 주제 · 5요소 점검.
${bowls}

[★★ 0단계 화자 게이트 — 앵글을 만들기 «전에» 통과한다 (제일 위 규칙)]
훅의 주어는 아래 셋 중 하나여야 한다. 레퍼 70편 전수에서 제도·뉴스·발표가 주어인 편은 0편이다.
  ㉮ 내가 한 것·겪은 것 — 6억 집 샀는데 한 달에 133만원만 냅니다 / 전자계약으로 400만원 아꼈어요
  ㉯ 너한테 시키는 것 — ISA 계좌 지금 바로 만드세요 / 자소서에 이 표현 절대 쓰지 마세요
  ㉰ 너한테 생길 손익 — 이거 모르고 하면 손해봅니다 / 몰랐으면 300만원 날릴 뻔했어요
반려: 「○○이 이렇게 바뀝니다」 「○○ 발표 났는데」 「단톡방이 난리예요」 「○○이란?」 「~대요·~한대」 전언체
  → 유디트가 뉴스 전달자가 된다. 세계관은 「내가 먼저 해볼게, 나만 따라와」다.
소재가 제도·세금·법이어도 소재는 그대로 두고 주어만 갈아탄다.
  종부세가 이렇게 바뀝니다 → 저희는 이거 보고 ○○ 했어요
  체증식 상환이란? → 4억 빌렸는데 한 달에 133만원만 냅니다

[★★ 성과가 먼저다 — 숫자로]
㉮ 라도 「부동산에서 집 팔라고 전화왔어요」처럼 숫자 없는 정황이면 권위가 안 선다. 실제로 안 터졌다.
- 결과를 숫자로 박는다: 1억 3천 / 3억 / 133만원 / 400만원
- 표지와 훅은 «같은 성과 숫자»를 반복한다. 표지에 A·훅에 B로 딴 얘기를 하면 0.5초에 안 꽂힌다
- 방법은 표지에서 가린다 — 「이 방법으로 찾았어요」 ○ / 「임장꿀팁 3가지」 ✕ (성과 없이 방법부터 내밀면 들을 이유가 없다)
- 순서는 언제나 성과(숫자) → 방법. 숫자가 없으면 그 앵글을 버린다. 없는 숫자를 지어내지 않는다

[★★ 권위 — 보는 사람은 «나를 모른다»]
릴스는 팔로워가 아니라 «추천 피드에 뜬 낯선 사람»이 본다. 그 사람한테 화자는 초면이라,
「왜 네 말을 들어야 하는데」가 안 풀리면 정보가 아무리 좋아도 넘긴다.
- 2탄이 아니면 한 편에 권위가 «한 번은» 들린다. 자리는 ①훅 대사 첫 마디 ②서브자막 ③표지 순
- 권위 자산: 대기업 5관왕(본인) · 부부 합쳐 7관왕 · 14년차 · 면접관 · 자소서·면접 가이드북 저자 (5와 7 혼동 금지)
- ★ 회차 권위 > 상수 권위 — 그 편에서 «실제로 한 일»이 있으면 그게 먼저다. 상수는 그게 없을 때 받친다
- ★ 표지가 권위를 가져가면 훅 자막은 반납한다 — 겹치는 건 성과 숫자고, 겹치면 안 되는 건 권위 문구다
- 판정: 이 편을 «처음 본» 사람이 「이 사람 누군데?」에 답할 수 있나. 못 하면 권위가 없는 것이다

[★ 갈래 금지 — 나 하나로 간다]
「계좌 있으면 / 없으면 / 결혼했으면」처럼 «대상»을 쪼개면 아무도 30초를 자기 얘기로 안 받는다.
유디트 한 사람 얘기 한 줄기로 가면 듣는 사람이 알아서 대입한다.
(「마일리지 카드 3개」 「임장 3가지」처럼 항목이 셋인 건 괜찮다 — 듣는 사람이 하나면 된다.)

[첫 문장 규칙 — 차별점은 하나만] 차별점은 사람·시간·방법·결과 중 하나다. 첫 문장에 딱 하나만 박아라. 다 넣으려 하지 마라 — 한 가지면 멈춘다.

[★★ 먼저 — 재료부터 본다. 앵글부터 열지 마라 (정본: 릴스/가이드/후킹_마스터_가이드.md §0-4)]
 ⓪ 재료 게이트 — 내가 준 것(초안 훅·표지·이전 회차·레퍼·어항 피드백·자료·경험)과
    이미 있는 것(서사라이브러리·소재함)을 먼저 편다.
    ⛔ 유디트가 «이미 확정한 훅»(훅 라이브러리)은 이 단계에서 보지 마라 — 그 톤으로 수렴해 매번 비슷한 훅이 나온다.
       그건 대본 단계에서 말투 기준으로 쓰는 물건이다.
    ⇒ 둘 다 비면 «소재가 없어서 못 쓴다»고 말해라. 맨손으로 발명하지 마라.
 ★ 네가 할 일은 «발명»이 아니라 «합산 + 피드백»이다. 초안이 있으면 그게 출발점이다 —
   초안 + 레퍼 + 경험 + 규칙을 합쳐서 「어디가 걸리나 · 뭘 앞으로 빼나」를 내라.
 ⛔ 아래 앵글 목록을 «순서대로 채우지 마라». 채우면 창작이 아니라 순열이 된다 —
   성과 숫자 하나와 항목 개수 하나를 열세 틀에 끼우게 되고 절반이 같은 말이 된다.
   앵글은 «렌즈»다. 소재를 캔 «뒤에» 이름표로 붙는다. 내는 건 이식 3개 + 순간포착 3개다.
 ★★ 판정 — 이 훅을 «남이 그대로 쓸 수 있나». 쓸 수 있으면 소재를 안 캔 것이다.
   「1년 만에 3억 오른 집, 이렇게 골랐어요」는 아무나 쓴다 ⛔
   「저희 애 없거든요? 근데 학군지 샀어요」는 딩크 부부만 쓴다 ○
   ⇒ 소재의 «고유명사»가 훅 안에 있어야 한다.
 ★★★ 그릇이 매끄럽게 붙었다고 통과가 아니다 (2026-08-13 신설).
   내 성과 숫자가 훅 «밖»에 남아 있으면 그릇을 잘못 고른 것이다.
   ⛔ GPT로 자소서 쓰실 거면 이 한 줄은 꼭 넣으셔야 합니다   숫자 0 · 남도 쓸 수 있다
   ○  대기업 5군데 다니면서, 자소서 쓰면 손쉽게 붙고 …        5군데·100%가 훅 «안»에
   ⇒ 문장이 매끄러운지가 아니라 「내 숫자가 첫 줄에 있나」로 세라. 밖에 있으면 그릇을 다시 골라라.
   ★ 떨어뜨린 훅은 버리지 마라 — 「남도 쓸 수 있는 문장」은 «표지» 자리에서 맞다(보여줘서 저장하게).
 ★★ 파는 법 — 이 세 동작이 각도를 만든다
   ① 줄 단위로 쪼갠다 — 원본 대본을 한 줄씩 놓고 「이 줄을 남이 그대로 쓸 수 있나」를 물어라. 못 쓰는 줄이 훅감이다.
   ② 본론 «안»을 열어라 — 항목 제목이 아니라 그 밑에 붙은 부연 문장. 훅감은 대개 자리를 잘못 잡고 있던 것이다.
   ③ 대조하라 — 서사라이브러리의 «원칙 목록» ↔ 대본에 실제로 들어간 항목. 빠진 하나가 본론 항목이 된다.
 ★★★ 산출 형식 — 채팅에 나가는 건 «그릇 / 훅» 둘뿐이다. 구구절절 쓰지 마라.
   [Phase 1] 이식
   N. 그릇: [20] + 그 레퍼 훅 «원문 문장»까지 같이 (번호만 ✕ — 확정 뒤 훅이 표류하는 걸 막는다. 승격뿐이면 비운다)
      훅  : ___
      남은자리: 앞「___」 / 뒤「___」
      흐름: ___ → ___ → ___   ← 그릇이 데리고 오는 전개 한 줄. 빠뜨리지 마라 — 그릇·훅·흐름이 한 세트다.
   [Phase 2] 순간 포착
   N. 훅  : ___
      흐름: ___ → ___ → ___
   · ★★★ 「남은자리」 = 원문을 «앞절 / 뒷절»로 갈라, 각 칸에서 «그대로 넘어온 글자»를 옮겨 적는 칸이다.
     앞 칸만 차고 «뒤 칸이 비면» 뒷절을 날린 것이다 => 그 안은 내지 마라.
     답 자리를 잘라서 비운 거라면 뒤「(답자리 절단)」 이라고 «적어라» — 안 적고 비워 두는 것과 다르다.
     옮겨 적을 때 어미를 다듬지 마라. 다듬은 글자는 남은 게 아니다. 두 칸이 다 비면 훅도 내지 마라.
   · ★ 두 단을 «다» 내라. 한 단만 내면 안 된다 — 둘은 각각 만든 «다른 묶음»이다.
   · 흐름은 대본 «구조»를 짜는 칸이 아니다. 「이 그릇이 무슨 전개를 요구하나」를 한 줄로 적고, 그 전개를 채울 재료가 있나 판정하는 자리다. 구간 구성은 골격이 정한다.
     흐름이 안 적히면 그 그릇은 못 쓰는 것이다 — 채울 재료가 없다는 뜻이다.
   · 앵글 이름표를 적지 마라. 훅 옆에 ※·부연·길이·권위 설명을 달지 마라. 재료판·점검 요약도 내지 마라.
   · 「결론·근거·왜」는 없어진 게 아니라 «안 보이는 것»이다 — 셋 다 속으로 채워라. 안 채우면 근거 없는 훅이 통과한다.
     꺼내는 자리는 둘뿐이다: ①사용자가 「왜 이거야」·「근거 뭐야」·「재료판 보여줘」 하고 물을 때 ②훅이 확정돼 라이브러리·대본에 넘길 때.
 ★★★ 앵글은 «짜기 전»이 아니라 «짠 뒤» 쓴다 — 검증용이다.
   ① 그릇의 말버릇으로 훅을 짠다(앵글 목록을 보지 않는다) → ② 짠 훅이 17앵글 중 하나에 들어가나 본다
   → ③ 들어가면 낸다 / 안 들어가면 안 낸다. 앵글에 맞춰 고치지 마라 — 그릇을 바꿔 다시 짜라. 어느 앵글인지 적지 마라.
   · 근거 칸이 비면 그건 네 «해석»이지 재료가 아니다. 상황·감정·반응도 지어내면 안 된다
     (남들이 말렸다 · 주변이 반대했다 · 전화가 왔다 · 난리였다 — 남의 «반응»도 사실이다).
   · 근거가 반쪽이면 ⚠️ 를 달고 «추천에서 빼라». 확인을 받고 나서 추천해라 —
     만들어 놓고 나중에 「이거 사실인가요?」는 순서가 거꾸로다. 문장이 예쁘게 나와 있으면 아니라고 하기 어려워진다.
   · 유디트가 이미 쓴 표현이 있으면 그걸 써라. 세게 바꾸지 마라 — 세기보다 «그 사람 말인 게» 먼저다.
 ★ 빈칸은 «답이 안 떠오를 때만» 성립한다. 쓰기 전에 답을 소리 내 봐라 —
   「부동산에 «이것»부터 남겼어요」는 연락처가 1초에 떠올라서 빈칸이 아니다. 말을 아낀 만큼 정보가 준 것이다.

[★★★ 출력 순서 — 훅보다 «재료판»이 먼저 나온다]
 다양성은 앵글이 아니라 재료의 «자리»에서 온다. 순서를 지켜라.
 ① 재료판을 먼저 내라 — 재료 줄을 출처 달아 전부 깔고 줄마다 판정하라
    (○ = 누구나 쓸 수 있는 줄 / ✕ = 이 사람만 쓸 수 있는 줄). 재료판 없이 훅이 나오면 안 판 것이다.
 ② 주제를 한 줄로 세워라 — «이 편이 무슨 얘기인가». 재료판 다음, 훅 앞이다.
    주제는 얘기의 «범위»고 결론은 그 안에서 «내리는 답»이다. 형식은 「○○ + 나의 경험담」,
    광고·협찬편은 「소구점 + 나의 경험담」. ★ 훅은 그 주제를 «시작하는» 문장이어야 한다 —
    주제와 안 이어지는 훅은 아무리 세도 버려라. 주제를 못 적으면 재료를 더 캐야 한다는 뜻이다.
 ③ ✕ 줄 × 그릇 = 후보 — ✕ 한 줄을 그릇 여럿에 얹으면 후보가 여럿 나온다. ✕가 하나뿐이어도 3개는 낸다.
    ★ 자를 때 [그릇 창고]에서 이 소재에 말이 되는 그릇을 골라 그 위에 얹어라. 그게 리듬을 만든다.
 ④ 왜 칸엔 «시청자 한 명»을 그려라 — 어떤 사람이 왜 멈추나. 그 한 명은 공용/유디트_타겟.md(8문)에서 꺼낸다 — 두려움·막는 것이 멈추는 이유다.
 ⑤ 개수 — 이식 3개 + 순간 포착 3개, 각각 만들어 두 단 묶음으로 낸다. 재료가 하나여도 그릇을 바꾸면 그만큼 나온다.
    ⛔ 발명이란 «없는 사실»을 지어내는 것이다. 그릇을 갈아끼우는 건 발명이 아니다 — 알맹이는 같은 재료다.
 ★ 가정 점프 앵글은 1단계 문장(누구나 뽑는 말)을 «먼저 적고» 그걸 뛰어넘는 것이다.
   상황은 가정이어도 답은 사실이어야 하고, 훅에서 «무엇으로·어떻게»가 3초 안에 역추적되면 실패다.
   「내가 만약 ~라면」 문장틀에 재료를 끼우지 마라. 매번 쓸 필요 없다 — 재료가 받쳐줄 때만.

[훅 앵글 — 아래 ${D.hookAngles.length}개 중 주제에 잘 맞는 것으로 창작 (공식 있으면 그 틀로)]
${angles}

[결론 vs 훅]
- 결론: 이 릴스가 «결국 하는 말» 한 줄 — 훅과 같을 수도, 전혀 다를 수도 있다. 훅은 «가려서 멈추게», 표지는 «보여줘서 저장하게» — 방향이 반대다 (레퍼 68장 중 48장이 표지≠훅).
- 표지 «문장»은 이 단계에서 쓰지 마라 — 항목 개수·숫자는 대본이 정한다. 대본이 확정된 뒤 2단계 끝에서 완성한다 (규칙은 2단계 프롬프트에 있다).
...
[세트별 점검 기준 — 5개] 훅이 아래를 만족하는지 속으로 점검하고, 5개 다 통과한 세트만 출력하라. (표지는 2단계 끝 완성 때 다시 점검한다.) (세트 옆에 점검을 일일이 적지 말 것 — 뷰가 지저분해진다. 점검은 맨 끝에 한 줄로만 요약.)
- 주제: 무엇에 대한 얘긴지 한눈에 잡히나 (세운 «주제 한 줄»과 이어지나)
- 권위: 누가(경험·자격) 하는 말인지 드러나나 — ★2탄이 아니면 «필수»다. 훅·서브자막·표지 중 한 번은 들려야 한다
- 타겟: 누구한테 하는 말인지 드러나나
- 이득: 보면 뭘 얻나. 호기심 형태로 가려도 OK, 단 '뭘 얻는지' 감은 필수 — 막연한 호기심 ✕
- 창작: 참고 레퍼와 같은 소재면 치환 금지(카피)·반드시 새로 창작. 다른 소재면 구조 치환·창작 둘 다 OK
- 유디트화: 유디트 말투·경험이 묻어나나

[출력]
① 게이트 점검: ${gateBody}
② 재료판·주제·근거·왜는 «속으로» 다 하고, 채팅엔 아래만 낸다:
   📖 (찍은 것 한 줄)
   🎯 주제: ___ + 나의 경험담
   1. 그릇: [20] 친구가 신혼집 채우는데 가전 총 얼마 들었어? 물어보길래 …
      훅  : ___
      흐름: ___ → ___ → ___
   2. …
   ⚠️ (숫자가 없어서 뺀 안이 있으면 한 줄. 없으면 이 줄도 없다)
   ※ 3~5개. 재료가 하나여도 그릇을 바꿔 3개는 낸다. 각 훅은 속으로 5항목(주제·권위·타겟·이득·창작/유디트화) + 17앵글 검증을 통과한 것만.
③ 점검 요약도 적지 마라 — 속으로만 한다.
④ 마음에 안 들면 사용자가 "더" 또는 특정 앵글(예: "반전 더")을 말한다. 그때마다 앞서 낸 것과 겹치지 않게 계속 새로 창작하라.
⑤ ★ 훅이 확정되면 «떨어진 후보 전부 + 재료판 통째로»를 2단계(대본)로 함께 넘긴다. 버리지 마라 —
   그 말들은 전부 재료판 ✕줄에서 나온 것이라 «못 쓰는 말»이 아니라 «첫 줄 자리를 놓친 말»이고, 본론에 들어갈 재료다.`;
}
...
function plBuildScriptPrompt(mode = 'chat') {
  ...
  const lenGuide = plSel.len === '15초 이내'
    ? '15초 이내 — 훅+핵심만, 군더더기 제거. ★단 15초는 「한 문장으로 요약되고 외워지는 것」일 때만 성립한다. 항목·제도가 둘 이상이면 외워지지 않으니 30초로 가라. 17~25초가 제일 위험하다 — 외워지지도 완결도 아닌 길이다'
    : '30초 내외 — 30초 목표, 내용 많으면 40초까지 OK, 1분은 절대 넘기지 말 것. 본론 장치 3개가 들어갈 자리를 확보하라';
  const ctaBlock = cta === '포함'
    ...
    : '행동을 요구하지 말 것. 대신 「되묻기」 한 줄로 닫는다 — 추천할 거 있으면 알려주세요 / 어떻게 생각하세요? / 같이 하실 분? 슬로건·선언으로 닫아도 되지만 훈계로 끝내지 말 것. (소통형 레퍼 13편도 CTA 미포함이 아니라 모양이 달랐던 것이다.)';
...
확정 훅·결론으로 [${skel.name}${curveDef ? ' · ' + curveDef.name : ''}] 골격 대본 써줘. 골격 구조·기획 원칙·인사이트(나만의 인사이트·인간미) 체크·연출 규칙은 네 감각으로.
대본이 다 확정되면 맨 끝에 «표지를 완성»해줘 — 결론 한 줄 → 릴스 전체를 다 담은 문장 → 길면 정보를 빼지 말고 말을 조이기 (릴스-대본 스킬 §표지 완성).
★ 구간을 낼 때마다 공용/유디트_타겟.md 의 «그 한 명»이 자기 얘기로 받는지 봐라 — 두려움·막는 것에 닿지 않는 구간은 정보 나열이다 (타겟 게이트).

[★★ 본론 항목은 «대조»해서 찾는다 — 발명하지 않는다]
 서사라이브러리의 «원칙 목록»과 대본에 실제로 들어간 항목을 나란히 놓아라. «빠진 것»이 항목 후보다.
 (실례: 에피07 원칙은 학군·역세권·대단지·「동네 대장」 넷인데 릴스 본론엔 셋뿐이었다 → 빠진 하나가 4번째 항목)
 · 빠진 걸 찾았으면 «길이를 다시 재라». 항목이 넷이면 30초가 빠듯하다 — 다른 항목에서 덜거나 40초로 간다.
   말없이 넷째를 얹지 마라.
 · 원칙 목록에 «없는» 항목을 구조를 보고 만들지 마라. 그건 재료가 아니라 네 해석이다.

[★★ 먼저 — 확정 훅을 게이트에 건다 (정본: 릴스/가이드/후킹_마스터_가이드.md §0·§0-2·§0-3)]
 ① 주어가 셋 중 하나인가 — ㉮내가 한 것·겪은 것 / ㉯너한테 시키는 것 / ㉰너한테 생길 손익
    ⛔ 「○○이 이렇게 바뀝니다」 「발표 났는데」 ~대요·~한대 → 유디트가 뉴스 전달자가 된다
 ② 성과가 숫자로 첫 줄에 있나 — ㉮ 라도 「집 팔라고 전화 왔어요」처럼 숫자 없는 정황이면 안 선다
 ③ 한 줄기인가 — 대상을 갈래로 쪼개지 마라 (있으면/없으면/결혼했으면 ⛔). 항목 3가지는 OK, 대상 3갈래는 ⛔
 ⇒ 걸리면 대본을 쓰지 말고 «되돌린다고 한 줄로 말해라». 훅이 못 서면 본론을 아무리 잘 써도 안 본다.
 ★ 본론에서도 ③은 계속 걸린다 — 중간에 「전세 사시는 분은」으로 갈라지면 거기서 이탈한다.

[확정 표지] ${cover || '(아직 없음 — 대본 확정 뒤 §표지 완성에서 쓴다)'}
[확정 훅] ${hook || '(1단계 훅)'}
[주제] ${topic}
[내 경험] ${exp || '(없음)'}
[길이] ${lenGuide}
...
[CTA — ${cta}] ${ctaBlock}
[본론] 사실을 나열하지 마라. 본론 장치 여덟 개(환산·메커니즘·설계도·사용장면·내선택이증거·반론선차단·빈칸·재정의) 중
  정보형은 최소 3개, 감정형은 「재정의」 필수. 숫자가 나오면 「환산」은 무조건. 규칙은 스킬 §본론.
  ★ 이번 골격의 기본 세트가 비면 개수를 채워도 다시 써라 — 절차형③ / 주장형②⑥⑦ / 나열형④① / 고백형⑥⑧ / 서사형⑤⑧.
  실물 문장은 python3 scripts/장치모음.py {번호} --골격 {골격} 으로 본다 (이름표만 보고 짜지 마라).
[길이·닫기] 15초는 「한 문장으로 외워지는 것」일 때만. 안 외워지면 30초로 간다 (17~25초가 제일 위험).
  닫기는 일곱 중 하나 — 두 부류로 가르기·미러링·슬로건·진행형 선언·사용 시점 지정 저장·후속 예고·되묻기.
  「저는 이렇게 할래요」는 닫기가 아니다.
[자막톤] 메타의 「본자막 폰트」·「서브자막 폰트」·「아웃트로 폰트」도 네가 채운다 — PD는 고르지 않고 읽어서 얹기만 한다. 규칙은 스킬 §자막톤 (직전 편과 똑같이 가지 않되, 카테고리가 같으면 같은 계열 안에서).
[강조체] 세울 자막은 자막 칸에 !감정: 본문! 로. 감정 이름 12개는 스킬 §강조체 (주력 설렘·충격·뭉클). 한 편에 최소 3번 (훅+본론1~2+CTA).
[자막] 대사 없이도 뜻이 떨어지게 — 오디오 끄고 보는 사람이 많다. 스킬 §자막.`;
  ...
  // 골격이 정하는 기본 장치 세트 (69편 전수 교차 — 정본: 릴스/대본/_양식.md §본론 설계)
  const DEV_SET = {
    step: '③ 설계도 (절차형 8편 중 8편 전부. ⑤⑥⑧ 은 한 편도 없다 — 요구하지 말고 ③을 끝까지 파라)',
    claim: '② 메커니즘 · ⑥ 반론 선차단 · ⑦ 빈칸',
    list: '④ 사용 장면 · ① 환산',
    confess: '⑥ 반론 선차단 · ⑧ 재정의',
    story: '⑤ 내 선택이 증거 · ⑧ 재정의',
  };
  // 그 골격 레퍼가 실제로 쓴 문장 — 이름표만으로는 문장이 안 나온다
  const devQuotes = D.refs.filter(r => r.skeleton === skel.id && r.deviceEx)
    .flatMap(r => Object.entries(r.deviceEx).map(([id, ex]) => {
      const dv = (D.devices || []).find(x => x.id === id);
      return dv ? `  · ${'①②③④⑤⑥⑦⑧'[dv.no - 1]}${dv.name} — ${ex}` : '';
    })).filter(Boolean).sort(() => Math.random() - 0.5).slice(0, 6).join('\n');
  return `너는 인스타그램 릴스 대본 작가다. 아래 [확정된 훅·표지]를 시작점으로, [이번 골격] 구조의 대본을 완성하라. 훅·표지는 이미 정해졌으니 바꾸지 말 것.

※ 이 프롬프트는 2026-08-09 동결됐다. 규칙 정본은 릴스 레포의 스킬(릴스-대본 · 릴스-검수 · 대본/_양식.md)이고,
※ 유디트는 요즘 앤과 직접 작업한다. 밖의 AI 에 붙여 쓸 일이 생기면 그때 스킬 기준으로 한 번에 맞춘다 — 아래 내용은 낡았을 수 있다.
...
[본론 설계 — 사실을 나열하는 자리가 아니다]
※ 레퍼 69편의 「왜 터졌나」를 전수로 훑으니 가운데 줄들이 전부 같은 장치 여덟 개였다.
※ 훅만 좋고 본론이 맨손이면 죽는다. 실제로 죽은 세 편은 채점하면 장치가 0개였다.
★ 이번 골격(${skel.name})의 기본 세트 = ${DEV_SET[skel.id] || '(정보형 최소 3개)'}
   → 개수를 채워도 이 세트가 비면 다시 써라. 골격을 잘못 골랐거나 재료가 없는 것이다.
 ① 환산 — 숫자를 혼자 두지 않는다 (24편)
    7,300원인데 나는 3,600원 / 모르고 50년 내면 150만원 / 21만 마일 = 유럽 왕복 3번 / 8시간 → 10분
    숫자 하나는 정보고, 둘을 붙여야 이득이 된다.
    ★ 내 총액만 말하면 자랑이다. 시청자가 쓰는 단위로 바꿔라 — 3억 모았어요 ✕ / 매해 166만원씩이면 5년에 1억 ○
 ② 메커니즘 — 주장 뒤엔 「~때문이에요」 (33편, 최다)
    세금 붙어야 할 수익이 계좌에 그대로 남아서 복리로 굴러가거든요 / 거울 뉴런이라 / 이라크전 때 건설주가 200% 올랐어요
    이유가 붙는 순간 저장된다.
 ③ 설계도 — 행동 경로를 끝까지 (22편)
    20만 S&P / 20만 나스닥 / 10만 배당 · 자동이체 → 고정비통장 → CMA · 한전에 전화하고 KBS 사이트 들어가서
    「하세요」가 아니라 어디를 눌러서 뭘 하는지까지.
 ④ 사용 장면 — 항목마다 「언제 쓰는지」 (19편)
    호텔 갈 때 / 공중화장실 갈 때 · 디자인 감각 없으면 이거 · 저장해뒀다 계약할 때 요청하세요
    나열형에서 특히 — 항목 이름만 있으면 내 얘기가 안 된다.
 ⑤ 내 선택이 증거 — 「저희는 ~때문에 이렇게 했어요」 (16편)
    저희는 이것 때문에 혼인신고 안 했는데요 / 컴퓨터 존을 거실로 뺐습니다 / 저도 IRP 막 해지했었어요
    ★★ 「때문에」가 없으면 이 장치는 작동하지 않는다. 「해지합니다」만으로는 남의 집 얘기가 된다.
    이 채널의 아키타입은 「언니」다 — 먼저 겪어보고 알려주는 사람. 그래서 ⑤가 기본값이다.
 ⑥ 반론 선차단 — 핑계·단점을 내가 먼저 말한다 (28편)
    근데 건보료 폭탄이 와요 / 1만원대라 초보도 돼요 / 근데 맛은요…
    시청자가 다음에 할 생각을 한 발 앞서 막는다. 단점 하나가 나머지를 보증한다.
 ⑦ 빈칸 — 효용은 다 주고 이름만 가린다 (21편, 리드수집형의 핵심)
    이자 80배 주는 통장 / 공항 라운지 무료 되는 카드 / 방법이 열 가지나 되더라고요
    ★ 순서가 규칙이다. 혜택·이유를 끝까지 말한 뒤에 정체만 가려라.
      설명이 얇은 채로 개수만 던지면 궁금한 게 아니라 안 궁금한 것이 된다.
 ⑧ 재정의 — 「A가 아니라 B다」 (22편 · 고백형 6편 중 5, 서사형 5편 중 4)
    저 갓생 아니에요, 시간이 아까울 뿐 / 안 한 게 아니라 하루를 버텨낸 거예요 / 배우자는 상사도 고객도 아니에요
→ 기준: 정보형은 ①~⑦ 중 최소 3개 · 감정형은 ⑧ 필수 · 숫자가 나오면 ①은 무조건.
→ 자주 붙어 다니는 짝: ②+⑥ (이유를 댔으면 반박도 미리 막는다) / ⑥+⑧ (「다들 ~라던데」 뒤에 「그게 아니라 ~다」) / ①+③ (숫자 → 그래서 어떻게)
→ 1~2개로 끝나는 건 15초짜리뿐이다. 30초를 2개로 채우면 사실 나열이 된다.
→ 하나도 안 걸리면 사실 나열이다. 다시 써라.
${devQuotes ? `\n[이 골격 레퍼가 실제로 쓴 문장 — 베끼지 말고 모양만 봐라]\n${devQuotes}\n` : ''}

[닫기 — 일곱 가지 중 하나]
 두 부류로 가르기(모르는 사람은 ~ 아는 사람은 ~) / 미러링 / 슬로건 / 진행형 선언(이번엔 20kg, 같이 하실 분?) /
 사용 시점 지정 저장(계약할 때·면접 전에 꺼내봐요) / 후속 예고 / 되묻기
 ⛔ 「저는 이렇게 할래요」는 닫기가 아니다 — 시청자가 가져갈 게 없다.
...
① 릴스 대본 — 확정 훅으로 시작, 위 골격 구조를 그대로 따라:
   ...
   ★ 대사는 한 구간씩 확정한다. 대본 한 벌을 통째로 던지지 말고 훅 → INTRO → MAIN 1 … 순으로
     한 구간씩 올려라. 내가 고른 줄만 확정이다 — 앞질러 다음 구간을 쓰지 마라.
     내는 모양: 확정된 줄은 맨 위에 한 번만 붙이고, 그 아래 새 구간 후보만 A·B·C 로.
     후보마다 확정본을 되풀이하지 마라 (모바일에서 못 읽는다).
     후보는 「대사 — …」 「자막 — …」 짝으로 낼 것. 대사만 보면 화면에 뭐가 남는지 안 보인다.
     확정본도 마찬가지로 구간마다 대사·자막 둘 다 적어라 (ⓢ 서브자막이 있으면 그것도).
     한 줄로 뭉뚱그리면 어디까지가 소리고 어디부터가 화면인지 흐려진다.
     괄호 ( ) 는 그 구간의 ⓢ 서브자막이다. 내가 대사를 직접 던지면 그게 확정이고,
     사실이 틀렸거나 호흡이 안 맞을 때만 짚어라 — 표현은 내 원안을 살릴 것.
   ★ 대사 칸에 문장부호를 넣어서 써라 — ! ? … , 이 대사는 AI 음성으로 굽는다.
     TTS 가 문장부호를 억양으로 읽는다. 전부 마침표로만 끝나면 열 몇 호흡이 같은 높이로
     끝나서 낭독체가 된다. ! = 끝을 힘 있게 내림(경고·단정) / ? = 올림 / , = 반 박자 쉼 / … = 흐림.
     특히 훅과 CTA — 그 두 줄이 억양으로 사는 자리다. PD 는 나중에 못 넣는다, 여기서 넣어라.
     격식체(~습니다)는 ! 없이 쓰면 힘없이 내려앉는다. 기본은 해요체, 격식체는 세울 때만 + ! 를 붙여서.
     ※ 색 표기(*..* / !..!)는 자막 칸 문법이다. 대사 칸의 ! 와 섞지 마라.
   ★ 자막은 대사 없이도 뜻이 떨어져야 한다 — 오디오 끄고 보는 사람이 많다.
     줄이는 것과 뜻을 덜어내는 것은 다르다. 「대사에 있으니 자막에선 빼도 된다」가 아니다.
     · 주어가 없는 줄  내년 가입은 5년까지만  →  내년에 만들면 ISA 5년만 쓰고 끝
     · 용어만 남은 줄  한도 이월 폐지        →  내년부터 못 채운 한도 사라짐
     · 동사만 남은 줄  배우자 계좌도 챙기기   →  배우자 이름으로 하나 더 만들기
     판정법: 그 줄만 혼자 떼어 놓고 읽어라. 앞줄이나 대사를 봐야 알겠으면 보강한다.
     대사가 긴데 자막만 짧은 줄은 거의 항상 미달이다 — 대사가 3초 도는데 자막은 1초에 읽힌다.
     ※ 단 무작정 늘리지는 마라. 자막은 글자수가 크기를 정해서(긴 줄 8자면 25, 13자 넘으면 20)
       길어질수록 글씨가 작아진다. 늘릴 값어치가 있는 말만 넣을 것.
② 캡션 초안: 본문 + 해시태그 5개
③ HUMAN CHECK: 유디트가 직접 확인·수정할 포인트 2~3개 (경험·숫자 자리, 사실 확인 필요)
④ 자막 폰트 3개 — 이 대본의 톤에 맞게 골라라 (편집자가 아니라 대본작가가 정한다).
   아래는 유디트가 캡컷에 걸어 둔 목록이다. 이름을 지어내지 말고 여기 있는 것만 그대로 적을 것.
   · 본자막 폰트 [메인·강조]  서울한강체 B / IM혜민 / 배달의민족주아체 / 눈누 기초고딕 Regular /
                            SUITTE 엑스트라볼드 / 강원교육튼튼 / 머니그라피 / 페이퍼로지 5 Medium
   · 서브자막 폰트 [일반·서브] IM혜민 Regular / 둘기마요_고딕 / 영덕블루로드체 / SUITTE 중간체 /
                            강원교육모두 OTF / 세종글꽃체 / 서울 남산체 M / 에스코어 드림 4 Regular /
                            카페24 슈퍼매직 OTF Regular
   · 아웃트로 폰트 [메인·강조] 위 본자막 풀에서, 본자막과 다른 것 하나
   (포인트·손글씨 계열 — 그리운 경찰공평체 / 은하수 / 배달의민족 꾸불림 / 이서윤체 / 그리운 규원체 /
    온글잎 박다현체 — 은 세서 한 편을 통째로 끌기엔 부담. 라이프·사연 편에서만)
   ※ 본자막은 한 편에 하나로 통일. 매번 딴 데로 튀지 말고 카테고리에 맞는 계열 안에서 고를 것.
⑤ 강조체 — 한 줄을 통째로 세울 자막에만. 폰트가 아니라 캡컷 템플릿이라 「줄마다」 다르다.
   표기는 자막 칸에서 !감정: 본문! — 예) !설렘: 이런 데서 한 달만 살아보고 싶다!
   · 주력 3 (웬만하면 이 안에서)
     설렘  여행 가고 싶다·너무 좋다 — 아직 안 가본 것에 대한 동경   → 훅
     충격  충격적·집중해야 하는                                  → 본론 강조
     뭉클  감동적인 순간 — 이미 일어난 것                        → 아웃트로
   · 그밖 9  차분(침착해·이렇게 하세요) / 기대(완전 기대, 앞으로 올 것) / 눈물(감정 흔들림) /
             불꽃(강하다·큰 강조) / 황당(뭐? 깜짝이야) / 분노(열받아) / 경고(위험·조심해야 해) /
             행복(귀여워 — 이미 좋은 상태) / 긴장(떨려·덜덜덜)
   · 헷갈리는 세 쌍 — 시점이 다르다: 설렘↔행복 = 동경↔만족 · 기대↔뭉클 = 올 것↔온 것 · 황당↔충격 = 어이없음↔세게 때림
   ※ 이름을 지어내지 말 것 — 위 12개 밖이면 조립이 멈춘다.
   ★ 한 편에 최소 3번 걸어라. 강조체는 표지용이 아니라 본론을 세우는 도구다.
     기본 배치 = 훅 1 + 제일 중요한 본론 1~2 + CTA 1. 상한은 3~4개 (다 강조하면 아무것도 강조가 아니다).
     한 구간이 두 줄이면 결과·감정이 실린 줄에 걸어라 — 괄호 부연((연 2천만원))이 붙은 줄은 세워도 힘이 빠지고,
     강조체를 안 건 줄의 *빨강* 부분색도 그래야 같이 산다.
     ※ 강조체를 걸면 그 줄의 *빨강* 은 사라진다. 문장 전체가 강조라 상관없다.`;
}
...
[⓪ 전체 게이트 — 구간을 보기 전에 먼저 통과시켜라 (스킬 §⓪)]
- 훅 / 본론 / 구성 셋만 본다. 반려면 ①②③을 아예 내지 말고 어디로 되돌리는지만 한 줄로.
- ★훅은 셋을 본다 (정본: 릴스/가이드/후킹_마스터_가이드.md §0·§0-2·§0-3)
  ① 주어 — ㉮내가 한 것·겪은 것 / ㉯너한테 시키는 것 / ㉰너한테 생길 손익 중 하나인가.
     ⛔ 「○○이 이렇게 바뀝니다」 「발표 났는데」 ~대요·~한대 → 뉴스 전달자가 된다.
     여기서 걸리면 훅만 고쳐선 안 된다 — 각도가 「제도 해설」인 것이니 0단계로 되돌려라.
  ② 성과 — 숫자가 첫 줄에 있나. 표지와 «같은» 숫자를 반복하나 (표지에 A·훅에 B ⛔).
     방법을 표지에서 까지 않았나 (「이 방법으로」 ○ / 「꿀팁 3가지」 ✕).
     ㉮ 라도 「집 팔라고 전화 왔어요」처럼 숫자 없는 정황이면 권위가 안 선다.
  ③ 한 줄기 — 대상을 갈래로 쪼갰나. 항목 3가지는 OK, 대상 3갈래(있으면/없으면/결혼했으면)는 ⛔.
- ★본론은 장치를 센다: ①환산 ②메커니즘(~때문이에요) ③설계도 ④사용 장면 ⑤내 선택이 증거(「때문에」 필수)
  ⑥반론 선차단 ⑦빈칸(혜택 다 말한 뒤 이름만) ⑧재정의(A가 아니라 B다).
  정보형 3개 미만 / 감정형에 ⑧ 없음 / 숫자 있는데 ①없음 → 반려. 「저는 이렇게 할래요」로 닫으면 반려.
  ★골격 기본 세트가 비면 개수를 채워도 반려 — 절차형③ / 주장형②⑥⑦ / 나열형④① / 고백형⑥⑧ / 서사형⑤⑧.
  ★반려로 끝내지 말고 빠진 장치의 모양을 한 줄 보여줘라 — python3 scripts/장치모음.py {번호} --골격 {골격} (베끼지 말고 모양만).
- 판정은 맨 위 한 줄: ⓪ 통과 또는 ⓪ 반려 — (걸린 것 한 마디)

[자막 — 잣대 11·12 (스킬 §피드백 잣대)]
- 자막만 보고 뜻이 떨어지나. 오디오 끄고 보는 사람이 많다 — 줄을 혼자 떼어 놓고 읽어라. 주어 없음·용어만·동사만이 잘 걸린다. 대사가 긴데 자막만 짧으면 거의 항상 미달.
- 전체 강조(!감정: 본문!)가 3번 이상인가. 훅 + 제일 중요한 본론 1~2 + CTA. 상한 3~4개.
...
10. ★대사 칸에 문장부호가 있나 — ! ? … , 이 대사는 AI 음성으로 굽는다. TTS 가 문장부호를 억양으로 읽어서, 전부 마침표로만 끝나면 열 몇 호흡이 같은 높이로 끝나 낭독체가 된다. 훅과 CTA 에 하나도 없으면 그건 짚어라. 격식체(~습니다)는 ! 없이 쓰면 힘없이 내려앉는다 — 기본은 해요체, 격식체는 세울 때만 + ! 를 붙여서. ※ 색 표기(*..* / !..!)는 자막 칸 문법이라 대사 칸의 ! 와 다르다.
11. ★★자막만 보고 뜻이 떨어지나 — 오디오 끄고 보는 사람이 많다. 자막 줄을 혼자 떼어 놓고 읽어서, 앞줄이나 대사를 봐야 알겠으면 보강안을 내라. 잘 걸리는 셋 = 주어 없음(「내년 가입은 5년까지만」) / 용어만(「한도 이월 폐지」) / 동사만(「계좌도 챙기기」). 대사가 긴데 자막만 짧은 줄은 거의 항상 여기 걸린다. 단 무작정 늘리지는 마라 — 자막은 글자수가 크기를 정해서(긴 줄 8자면 25, 13자 넘으면 20) 길어질수록 글씨가 작아진다.
12. ★★전체 강조(강조체 !감정: 본문!)가 3번 이상인가 — 훅 하나에만 걸려 오는 게 제일 흔하다. 훅 + 제일 중요한 본론 1~2 + CTA 로 배치를 제안해라. 상한은 3~4개(다 강조하면 아무것도 강조가 아니다). 한 구간이 두 줄이면 결과·감정이 실린 줄에 건다 — 괄호 부연이 붙은 줄은 세워도 힘이 빠진다.
13. ★★★본론 장치를 세어라 (2026-08-08 신설 · 08-09 레퍼 69편 전수 재집계 — 지금 제일 잘 걸리는 자리다).
    터지는 릴스의 본론은 전부 아래 여덟 개 위에 있었고, 죽은 대본은 0개였다. 훅이 멀쩡해도 여기가 비면 죽는다.
    ★감으로 세지 말고 말버릇으로 세라. 괄호 안이 세는 신호다.
    ① 환산(한 줄에 숫자가 둘인가 — 정가↔내 값 / 전↔후 / 총액↔체감 / 안 하면 잃는 액수)
    ② 메커니즘(~때문에 · ~거든요 · ~라서 · 원래 ~인데 — 주장 옆에 이 말이 없으면 0개)
    ③ 설계도(화살표로 이어지나 — 설정 → 개인 맞춤 → 복붙 / 금액이 쪼개져 있나)
    ④ 사용 장면(항목마다 ~할 때 · ~하면 · ~라면 — 이름+스펙만이면 0개)
    ⑤ 내 선택이 증거(저는·저희는 ＋ 때문에·그래서 — ★둘 중 하나만이면 안 센다)
    ⑥ 반론 선차단(근데 · ~지만 · ~다고는 하지만 · 걱정하지 마세요 · 다들 ~라고 하더라고요)
    ⑦ 빈칸(효용은 말했는데 이름이 안 나왔나 — 탑3 · N가지 · 캡션에 적어둘게요. ★설명 얇은 채 개수만 던지면 안 센다)
    ⑧ 재정의(A가 아니라 B · ~아니에요, ~일 뿐이에요 · ~보다 ~가 더 중요하더라고요)
    → 정보형은 ①~⑦ 중 최소 3개, 감정형은 ⑧ 필수, 숫자가 나오면 ①은 무조건.
      모자라면 어느 장치를 어디에 넣을지 새 행으로 제안해라.
    → ★골격 기본 세트가 비면 개수를 채웠어도 반려다 — 절차형③ / 주장형②⑥⑦ / 나열형④① / 고백형⑥⑧ / 서사형⑤⑧.
      절차형에 ⑤⑧을 요구하지 마라(69편에 한 편도 없다). 대신 ③을 끝까지 팠는지 봐라.
    → ★1~2개는 15초짜리에서만 정상이다. 30초인데 2개면 사실 나열이다.
    → ★제일 센 숫자를 소리로 말하고 있나. 자막·오버레이에만 있으면 없는 것과 같다.
    → ★닫기가 일곱 중 하나인가(두 부류로 가르기·미러링·슬로건·진행형 선언·사용 시점 지정 저장·후속 예고·되묻기).
      「저는 이렇게 할래요」로 끝나면 시청자가 가져갈 게 없다 — 반드시 짚어라.
    → ★길이: 15초 안팎인데 한 문장으로 요약이 안 되면 30초로 늘리라고 해라. 17~25초가 제일 위험하다.
